...
Tensor tril_indices_cuda(
  ...
  if (tril_size > 0) {
    ...
    AT_DISPATCH_ALL_TYPES_AND_UNIVERSAL_AND(at::ScalarType::Half, tensor.scalar_type(), "tril_indices_cuda", [&] {
      tril_indices_kernel<<<
...
Tensor triu_indices_cuda(
  ...
  if (triu_size > 0) {
    ...
    AT_DISPATCH_ALL_TYPES_AND_UNIVERSAL_AND(at::ScalarType::Half, tensor.scalar_type(), "triu_indices_cuda", [&] {
      triu_indices_kernel<<<
          dim_grid, dim_block, 0, at::cuda::getCurrentCUDAStream()>>>(
        tensor.data_ptr<scalar_t>(),
