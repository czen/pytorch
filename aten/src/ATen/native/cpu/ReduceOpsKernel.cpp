...
static void cumsum_cpu_kernel(const Tensor& result, const Tensor& self, int64_t dim) {
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND_UNIVERSAL_AND(kBFloat16, self.scalar_type(), "cumsum_out_cpu", [&] {
...
static void cumprod_cpu_kernel(const Tensor& result, const Tensor& self, int64_t dim) {
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND_UNIVERSAL_AND(kBFloat16, self.scalar_type(), "cumprod_out_cpu", [&] {
    cpu_cum_base_kernel<scalar_t>(result, self, wrap_dim, [&] (
      scalar_t* result_data, auto result_dim_stride,
      const scalar_t* self_data, auto self_dim_stride, scalar_t init_val) {
        // NOLINTNEXTLINE(bugprone-signed-char-misuse)
...
static void logcumsumexp_cpu_kernel(Tensor& result, const Tensor& self, int64_t dim) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_UNIVERSAL_AND(kBFloat16, self.scalar_type(), "logcumsumexp_out_cpu", [&] {
    cpu_cum_base_kernel<scalar_t>(result, self, wrap_dim, [&] (
      scalar_t* result_data, auto result_dim_stride,
      const scalar_t* self_data, auto self_dim_stride, scalar_t init_val) {
...
static void mean_kernel_impl(TensorIterator& iter) {
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND_UNIVERSAL(iter.dtype(), "mean_cpu", [&] {
...
static void std_var_kernel_impl(TensorIterator& iter, int64_t correction, bool take_sqrt) {
  AT_DISPATCH_FLOATING_TYPES_AND_UNIVERSAL_AND2(kHalf, kBFloat16, iter.dtype(), "std_cpu", [&] {
    binary_kernel_reduce(
        iter,
        WelfordOps<
            scalar_t,
...
static void prod_kernel_impl(TensorIterator& iter) {
  ...
  if (iter.dtype() == ScalarType::Bool) {
    ...
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND_UNIVERSAL(iter.dtype(), "prod_cpu", [&] {
      binary_kernel_reduce_vec(
          iter,
          [=](scalar_t a, scalar_t b)
              __ubsan_ignore_undefined__ -> scalar_t { return a * b; },
...
static void norm_kernel_tensor_iterator_impl(
  ...
  if (val == 0) {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND_UNIVERSAL_AND2(kHalf, kBFloat16, iter.input_dtype(), "norm_cpu", [&] {
    ...
    if (use_fast_path) {
      AT_DISPATCH_FLOATING_TYPES_AND_UNIVERSAL_AND(kBFloat16, iter.input_dtype(), "norm_cpu", [&] {
...
static void min_values_kernel_impl(TensorIterator& iter) {
  ...
  AT_DISPATCH_ALL_TYPES_AND_UNIVERSAL_AND3(kBFloat16, kHalf, kBool, iter.dtype(), "min_values_cpu", [&iter] {
    binary_kernel_reduce_vec(
      iter,
...
static void max_values_kernel_impl(TensorIterator& iter) {
  AT_DISPATCH_ALL_TYPES_AND_UNIVERSAL_AND3(kBFloat16, kHalf, kBool, iter.dtype(), "max_values_cpu", [&iter] {
    binary_kernel_reduce_vec(
      iter,
...
static void argmax_kernel_impl(TensorIterator &iter) {
  AT_DISPATCH_ALL_TYPES_AND_UNIVERSAL_AND2(kHalf, kBFloat16, iter.dtype(1), "argmax_cpu", [&] {
...
static void argmin_kernel_impl(TensorIterator &iter) {
  AT_DISPATCH_ALL_TYPES_AND_UNIVERSAL_AND2(kHalf, kBFloat16, iter.dtype(1), "argmin_cpu", [&] {
    if (is_reduce_lastdim(iter)) {
      using arg_t = std::pair<scalar_t, int64_t>;
