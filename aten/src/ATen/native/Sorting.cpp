...
std::tuple<Tensor&, Tensor&> kthvalue_out_impl_cpu(
  ...
  AT_DISPATCH_ALL_TYPES_AND_UNIVERSAL_AND(ScalarType::BFloat16, self.scalar_type(), "kthvalue_cpu", [&] {
...
std::tuple<Tensor&, Tensor&> median_with_indices_impl(
  ...
  AT_DISPATCH_ALL_TYPES_AND_UNIVERSAL_AND(ScalarType::BFloat16, in.scalar_type(), "median_out", [&] {
    auto loop = [&](char** data, const int64_t* strides, int64_t n) {
      for (const auto i : c10::irange(n)) {
...
Tensor median_impl(const Tensor& self, bool ignore_nan) {
  ...
  AT_DISPATCH_ALL_TYPES_AND_UNIVERSAL_AND(ScalarType::BFloat16, in.scalar_type(), "median_cpu", [&] {
