...
class TestBinaryUfuncs(TestCase):
    ...
    @ops(binary_ufuncs_with_references, dtypes=universal_types())
    ...
    @ops(binary_ufuncs, dtypes=universal_types())
    ...
    def test_div_rounding_nonfinite(self, device, dtype):
        ...
        exact_dtype = dtype != torch.bfloat16 and dtype not in universal_types()
        if exact_dtype:
    ...
    @dtypes(*product(get_all_dtypes_and_universal(include_complex=False), get_all_dtypes_and_universal(include_complex=False)))
    ...
    def test_maximum_minimum_float(self, device, dtype):
        ...
        if dtype == torch.bfloat16 or dtype in universal_types():
    ...
    @dtypes(*product(get_all_fp_dtypes_and_universal(), get_all_fp_dtypes_and_universal()))
    ...
    @dtypes(*product(get_all_dtypes_and_universal(include_complex=False),
                     get_all_dtypes_and_universal(include_complex=False)))
    def test_copysign(self, device, dtypes):
        def _test_copysign_numpy(a, b):
            ...
            types = [torch.bool, torch.bfloat16] + get_all_int_dtypes() + list(universal_types())
    ...
    @dtypes(*product(get_all_fp_dtypes_and_universal(),
                     get_all_fp_dtypes_and_universal()))
    ...
    @onlyNativeDeviceTypes
    @dtypes(*list(product(get_all_dtypes_and_universal(include_complex=False),
                          get_all_dtypes_and_universal(include_complex=False))))
    def test_heaviside(self, device, dtypes):
        ...
        input = np.array(rng.integers(-10, 10, size=10),
                         dtype=torch_to_numpy_dtype_dict[input_dtype if (input_dtype != torch.bfloat16 and input_dtype not in universal_types()) else torch.float64])
        input[0] = input[3] = input[7] = 0
        values = np.array(rng.integers(-10, 10, size=10),
                          dtype=torch_to_numpy_dtype_dict[values_dtype if (values_dtype != torch.bfloat16 and values_dtype not in universal_types()) else torch.float64])
    ...
    @dtypes(*product(get_all_dtypes_and_universal(), get_all_dtypes_and_universal()))
    ...
    @dtypes(*list(product(get_all_dtypes_and_universal(include_bool=False),
    ...
    # test for universal types is separate
    @skipIf(not TEST_SCIPY, "Scipy required for the test.")
