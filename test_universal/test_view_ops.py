...
class TestViewOps(TestCase):
    ...
    def test_view_dtype_new(self, device, dtype):
        ...
        def generate_inputs():
            yield make_tensor((4, 4, 64), device=device, dtype=dtype, low=-5, high=5)
            yield make_tensor((4, 4, 64), device=device, dtype=dtype, low=-5, high=5).permute(1, 0, 2)
            yield make_tensor((4, 64, 4), device=device, dtype=dtype, low=-5, high=5).permute(2, 0, 1)
            yield make_tensor((1, 5, 1), device=device, dtype=dtype, low=-5, high=5).expand(5, 5, 64)
            yield make_tensor((2, 5, 256), device=device, dtype=dtype, low=-5, high=5)[1::2, 1:, ::2]
            yield make_tensor((0, 5, 64), device=device, dtype=dtype, low=-5, high=5)
            yield make_tensor((), device=device, dtype=dtype, low=-5, high=5)
        ...
        if dtype.is_floating_point or dtype.is_complex:
            for view_dtype in [*get_all_fp_dtypes(), *get_all_complex_dtypes(), *universal_types()]:
                t = make_tensor((5, 5, 64), device=device, dtype=dtype, low=-5, high=5, requires_grad=True)
    ...
    def test_view_dtype_upsize_errors(self, device, dtype):
        ...
        for view_dtype in get_all_dtypes_and_universal():
            ...
            size_ratio = view_dtype_size // dtype_size
            a = make_tensor((4, 4, size_ratio + 1), device=device, dtype=dtype, low=-5, high=5)
            ...
            a = make_tensor((4, 4, size_ratio), device=device, dtype=dtype, low=-5, high=5)
    ...
    def test_view_tensor_split(self, device, dtype):
        a = make_tensor((40, 30), device=device, dtype=dtype, low=-9, high=9)
    ...
    def test_view_tensor_hsplit(self, device, dtype):
        t = make_tensor((4, 4, 4), device=device, dtype=dtype, low=-9, high=9)
...
class TestOldViewOps(TestCase):
    ...
    def test_broadcast_to(self, device, dtype):
        ...
        for s0, s1 in combinations(sizes, r=2):
            t = make_tensor(s0, device=device, dtype=dtype, low=-9, high=9)
            if dtype in universal_types():
    ...
    def test_reshape_view_semantics(self, device, dtype):
        tensor = make_tensor((15, 4), device=device, dtype=dtype)
        target = (20, 3)
    ...
    def test_tensor_split_sections(self, device, dtype):
        ...
        for input_size in input_sizes:
            a_base = make_tensor(input_size, device=device, dtype=dtype, low=-9, high=9)
